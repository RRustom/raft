...
type Harness struct {
  ...
  // a channel per server in cluster
  commitChans []chan CommitEntry

  // commit at i holds sequence of commits made by server i so far. Populated
  // by goroutines that listen on the corresponding commitChans channel
  commits [][]CommitEntry
...
// verify all connected servers have cmd committed with the same index.
// verify that all commands before cmd in the commit sequence match, which means
// all commands submitted must be unique positive ints.
// returns the numbers of servers that have this command committed, and its log index.
func (h *Harness) CheckCommitted(cmd int) (nc int, index int) {
  ...
  for i := 0; i < h.n; i++ {
    if h.connected[i] {
      if commitsLen >= 0 {
        // if this was already set, expect the new length to be the same
