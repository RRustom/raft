...
func (s *Server) Serve() {
  ...
  // create a new RPC server and register an RPCProxy that forwards all methods to n.cm
  s.rpcServer = rpc.NewServer()
  s.rpcProxy = &RPCProxy{cm: s.cm} //QUESTION: why pointer?
...
// close the server and wait for it to shut down properly
...
// disconnect server from the peer with peerId
...
func (s *Server) Call(id int, serviceMethod string, args interface{}, reply interface{}) error {
  ...
  // return error if this is called after shutdown (where client.Close is called)
  if peer == nil {
